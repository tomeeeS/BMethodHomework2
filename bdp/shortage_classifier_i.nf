...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(shortage_classifier_i),IsOnShortage)==(CurrentStocks: 1..5 --> 0..4000 & ii: dom(CurrentStocks) & MaxStocks: 1..5 --> 0..4000 & Shortages: 1..5 --> BOOL & NewCurrentStock: 0..4000 & !xx.(xx: dom(Shortages|>{TRUE}) => CurrentStocks(xx)<MaxStocks(xx)/2) & !xx.(xx: dom(CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2 => xx: dom(Shortages|>{TRUE})) | (assert(dom(Shortages|>{TRUE}) = {xx | xx: dom(CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2}) | dom(Shortages|>{TRUE}) = {xx | xx: dom(CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} ==> skip);(NewCurrentStock<MaxStocks(ii)/2 ==> (res:=Shortages;(ii: dom(res) | res:=res<+{ii|->TRUE})) [] not(NewCurrentStock<MaxStocks(ii)/2) ==> (res:=Shortages;(ii: dom(res) | res:=res<+{ii|->FALSE})));(assert(dom({ii}<<|Shortages|>{TRUE}) = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & !xx.(xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)<MaxStocks(xx)/2 => xx: {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2}) & dom(CurrentStocks<+{ii|->NewCurrentStock}) = dom({ii}<<|CurrentStocks)\/{ii} & !xx.(xx: dom({ii}<<|Shortages) => res(xx) = Shortages(xx)) & dom({ii}<<|Shortages|>{TRUE}) = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & (NewCurrentStock<MaxStocks(ii)/2 => res(ii) = TRUE) & (not(NewCurrentStock<MaxStocks(ii)/2) => res(ii) = FALSE) & dom(Shortages<+{ii|->TRUE}|>{TRUE}) = dom({ii}<<|Shortages|>{TRUE})\/{ii} & Shortages<+{ii|->FALSE} = {ii}<<|Shortages\/{ii|->FALSE} & dom({ii}<<|Shortages|>{TRUE}) <: dom(Shortages|>{TRUE})) | dom({ii}<<|Shortages|>{TRUE}) = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & !xx.(xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)<MaxStocks(xx)/2 => xx: {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2}) & dom(CurrentStocks<+{ii|->NewCurrentStock}) = dom({ii}<<|CurrentStocks)\/{ii} & !xx.(xx: dom({ii}<<|Shortages) => res(xx) = Shortages(xx)) & dom({ii}<<|Shortages|>{TRUE}) = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & (NewCurrentStock<MaxStocks(ii)/2 => res(ii) = TRUE) & (not(NewCurrentStock<MaxStocks(ii)/2) => res(ii) = FALSE) & dom(Shortages<+{ii|->TRUE}|>{TRUE}) = dom({ii}<<|Shortages|>{TRUE})\/{ii} & Shortages<+{ii|->FALSE} = {ii}<<|Shortages\/{ii|->FALSE} & dom({ii}<<|Shortages|>{TRUE}) <: dom(Shortages|>{TRUE}) ==> skip);(assert({xx | xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)<MaxStocks(xx)/2} = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & dom(Shortages<+{ii|->FALSE}|>{TRUE}) = dom({ii}<<|Shortages|>{TRUE})) | {xx | xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)<MaxStocks(xx)/2} = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & dom(Shortages<+{ii|->FALSE}|>{TRUE}) = dom({ii}<<|Shortages|>{TRUE}) ==> skip);(assert({xx | xx: dom(CurrentStocks<+{ii|->NewCurrentStock}) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2} = {xx | xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2}\/{xx | xx: dom({ii|->NewCurrentStock}) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2}) | {xx | xx: dom(CurrentStocks<+{ii|->NewCurrentStock}) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2} = {xx | xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2}\/{xx | xx: dom({ii|->NewCurrentStock}) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2} ==> skip));
  List_Substitution(Implementation(shortage_classifier_i),IsOnShortage)==(ASSERT dom(Shortages|>{TRUE}) = {xx | xx: dom(CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} THEN skip END;IF NewCurrentStock<MaxStocks(ii)/2 THEN res:=Shortages;res(ii):=TRUE ELSE res:=Shortages;res(ii):=FALSE END;ASSERT dom({ii}<<|Shortages|>{TRUE}) = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & !xx.(xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)<MaxStocks(xx)/2 => xx: {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2}) & dom(CurrentStocks<+{ii|->NewCurrentStock}) = dom({ii}<<|CurrentStocks)\/{ii} & !xx.(xx: dom({ii}<<|Shortages) => res(xx) = Shortages(xx)) & dom({ii}<<|Shortages|>{TRUE}) = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & (NewCurrentStock<MaxStocks(ii)/2 => res(ii) = TRUE) & (not(NewCurrentStock<MaxStocks(ii)/2) => res(ii) = FALSE) & dom(Shortages<+{ii|->TRUE}|>{TRUE}) = dom({ii}<<|Shortages|>{TRUE})\/{ii} & Shortages<+{ii|->FALSE} = {ii}<<|Shortages\/{ii|->FALSE} & dom({ii}<<|Shortages|>{TRUE}) <: dom(Shortages|>{TRUE}) THEN skip END;ASSERT {xx | xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)<MaxStocks(xx)/2} = {xx | xx: dom({ii}<<|CurrentStocks) & CurrentStocks(xx)<MaxStocks(xx)/2} & dom(Shortages<+{ii|->FALSE}|>{TRUE}) = dom({ii}<<|Shortages|>{TRUE}) THEN skip END;ASSERT {xx | xx: dom(CurrentStocks<+{ii|->NewCurrentStock}) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2} = {xx | xx: dom({ii}<<|CurrentStocks) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2}\/{xx | xx: dom({ii|->NewCurrentStock}) & (CurrentStocks<+{ii|->NewCurrentStock})(xx)+1<=MaxStocks(xx)/2} THEN skip END)
END
&
